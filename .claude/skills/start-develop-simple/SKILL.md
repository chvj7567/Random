...
사용자가 **명시적으로 호출했을 때만**, Random 의 표준 협업 흐름(`.claude/project.md` 의 「협업 흐름」)에서 **리뷰어 2명(design-reviewer / code-reviewer) 과 qa-simulator 를 모두 생략**하고 game-designer → gameplay-programmer → test-engineer 만으로 빠르게 돌린다. **프로토타입을 짧게 짜볼 때 시간 절약용**으로만 쓴다.
