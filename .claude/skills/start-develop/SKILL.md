---
name: start-develop
description: Use ONLY when the user explicitly invokes this skill by name. Runs Random's feature-development pipeline (game-designer through test-engineer) with a user-approval gate after the design doc. Do not auto-trigger from an ordinary feature request — explicit invocation required.
---
...
사용자가 **명시적으로 호출했을 때만**, Random 의 표준 협업 흐름(`.claude/project.md` 의 「협업 흐름」)을 한 번에 오케스트레이션한다. 기획서 단계에서 **멈춰 사용자 승인을 받은 뒤** 구현으로 넘어간다.
