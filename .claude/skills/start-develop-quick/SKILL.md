...
description: Use ONLY when the user explicitly invokes this skill by name, or when the user selects it from the main orchestrator's pipeline-choice prompt. Runs Random's lightest path — gameplay-programmer → code-reviewer only — for small bug fixes / renames / minor edits with no design or test work. Skips game-designer, design-reviewer, user-approval gate, test-engineer, qa-simulator, and superpowers 0·1 stages. Do not auto-trigger from an ordinary feature request.
---
...
사용자가 **명시적으로 호출했거나, 메인 오케스트레이터가 제시한 후보 중 사용자가 선택한 경우에만** 발동한다. Random 의 협업 흐름 4종 중 가장 가볍다 — `gameplay-programmer` 와 `code-reviewer` 만 돌린다.
...
2. **code-reviewer** 위임 → 코딩 룰(`.claude/rules/` Rule 00~04) 준수 + 사용자 의도와의 부합 검토.
...
- 코딩 룰(`.claude/rules/` Rule 00~04) · 프로젝트 범위·금지 사항(`project.md` / rules) 은 그대로 적용된다. 단계가 빠질 뿐 룰이 사라진 게 아니다.
